...
async function ensureTables() {
  try {
    ...
    try { await sql`ALTER TABLE ordenes_servicio ADD COLUMN IF NOT EXISTS sucursal_id TEXT`; } catch (e) {}
    try { await sql`ALTER TABLE reports ADD COLUMN IF NOT EXISTS sucursal_id TEXT`; } catch (e) {}
    try { await sql`ALTER TABLE reports ADD COLUMN IF NOT EXISTS orden_servicio_uuid TEXT`; } catch (e) {}
    ...
    try { await sql`CREATE INDEX IF NOT EXISTS idx_reports_order ON reports (cliente_id, orden_servicio_uuid, deleted_at)`; } catch(e){}
        ...
        try {
          ...
          if (table === 'assets') {
            ...
            switch (table) {
              ...
              case 'ordenes_servicio': await sql`INSERT INTO ordenes_servicio (id, data, uuid_sync, updated_at, created_at, cliente_id, sucursal_id, estado) VALUES (${id}, ${strData}, ${uuid_sync}, ${updated_at}, ${updated_at}, ${recordClienteId}, ${data.sucursal_id}, ${data.estado || data.data?.estado || data.data?.status || 'abierto'}) ON CONFLICT (uuid_sync) DO UPDATE SET id = EXCLUDED.id, data = EXCLUDED.data, updated_at = EXCLUDED.updated_at, cliente_id = EXCLUDED.cliente_id, sucursal_id = EXCLUDED.sucursal_id, estado = EXCLUDED.estado WHERE ordenes_servicio.cliente_id = EXCLUDED.cliente_id AND (EXCLUDED.updated_at > ordenes_servicio.updated_at OR ordenes_servicio.updated_at IS NULL)`; break;
        ...
        try {
          ...
          if (table === 'assets') {
            ...
            switch (table) {
              ...
              case 'ordenes_servicio': await sql`UPDATE ordenes_servicio SET id = ${id}, data = ${strData}, updated_at = ${updated_at}, cliente_id = ${recordClienteId}, sucursal_id = ${data.sucursal_id}, estado = ${data.estado || data.data?.estado || data.data?.status || 'abierto'} WHERE uuid_sync = ${uuid_sync} AND cliente_id = ${recordClienteId} AND (updated_at < ${updated_at} OR updated_at IS NULL)`; break;
        ...
        try {
          switch (table) {
            ...
            case 'ordenes_servicio': {
              if (!isAdminAuthUser((req as any).authUser)) throw new Error('Solo un administrador puede eliminar órdenes de servicio');
              const children = await sql`SELECT 1 FROM reports WHERE orden_servicio_uuid = ${del.uuid_sync} AND cliente_id = ${clienteIdSync} AND deleted_at IS NULL LIMIT 1`;
              if (children.length > 0) throw new Error('No se puede eliminar una orden que contiene informes');
              await sql`UPDATE ordenes_servicio SET deleted_at = ${ts}, updated_at = ${ts} WHERE uuid_sync = ${del.uuid_sync} AND cliente_id = ${clienteIdSync} AND estado NOT IN ('cerrado', 'firmado')`;
              break;
            }
      ...
      for (const record of records) {
        ...
        if (operation === 'delete') {
          ...
          switch (table) {
            ...
            case 'ordenes_servicio': {
              if (!isAdminAuthUser((req as any).authUser)) throw new Error('Solo un administrador puede eliminar órdenes de servicio');
              const children = await sql`SELECT 1 FROM reports WHERE orden_servicio_uuid = ${record.uuid_sync} AND cliente_id = ${clienteIdSync} AND deleted_at IS NULL LIMIT 1`;
              if (children.length > 0) throw new Error('No se puede eliminar una orden que contiene informes');
              await sql`UPDATE ordenes_servicio SET deleted_at = ${ts}, updated_at = ${ts} WHERE uuid_sync = ${record.uuid_sync} AND cliente_id = ${clienteIdSync} AND estado NOT IN ('cerrado', 'firmado')`;
              break;
            }
          }
        ...
        if (table === 'assets' || table === 'equipos') {
          ...
          switch (table) {
            ...
            case 'reports': await sql`INSERT INTO reports (id, data, uuid_sync, updated_at, cliente_id, sucursal_id, orden_servicio_uuid) VALUES (${id}, ${data}, ${uuid_sync}, ${updated_at}, ${clienteIdSync}, ${record.sucursal_id}, ${record.orden_servicio_uuid}) ON CONFLICT (uuid_sync) DO UPDATE SET id = EXCLUDED.id, data = EXCLUDED.data, updated_at = EXCLUDED.updated_at, cliente_id = EXCLUDED.cliente_id, sucursal_id = EXCLUDED.sucursal_id, orden_servicio_uuid = EXCLUDED.orden_servicio_uuid WHERE EXCLUDED.updated_at > reports.updated_at AND reports.cliente_id = ${clienteIdSync}`; break;
            ...
            case 'ordenes_servicio': await sql`INSERT INTO ordenes_servicio (id, draft_key, data, uuid_sync, updated_at, cliente_id, sucursal_id, estado) VALUES (${id}, ${record.draft_key || ''}, ${data}, ${uuid_sync}, ${updated_at}, ${clienteIdSync}, ${record.sucursal_id}, ${record.estado || record.data?.estado || record.data?.status || 'abierto'}) ON CONFLICT (uuid_sync) DO UPDATE SET id = EXCLUDED.id, draft_key = EXCLUDED.draft_key, data = EXCLUDED.data, updated_at = EXCLUDED.updated_at, cliente_id = EXCLUDED.cliente_id, sucursal_id = EXCLUDED.sucursal_id, estado = EXCLUDED.estado WHERE EXCLUDED.updated_at > ordenes_servicio.updated_at AND ordenes_servicio.cliente_id = ${clienteIdSync}`; break;
          }
        }
