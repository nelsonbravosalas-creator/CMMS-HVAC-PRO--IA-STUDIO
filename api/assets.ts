  ...
  try {
    ...
    if (method === 'POST') {
      ...
      await sql`
        INSERT INTO assets (tag, nombre, tipo, marca, modelo, serie, ubicacion, area,
          ...
          tecnicos, notas, cliente_id, sucursal_id, uuid_sync, updated_at, created_at, deleted_at)
        VALUES (
          ...
          ${d.uuid_sync || d.tag}, ${d.updated_at || now}, ${now}, ${d.deleted_at || null}
        )
        ON CONFLICT (tag) DO UPDATE SET
          ...
          sucursal_id = EXCLUDED.sucursal_id, deleted_at = EXCLUDED.deleted_at,
    ...
    if (method === 'DELETE') {
      ...
      const rows = await sql`
        ...
        WHERE tag = ${tag}
        RETURNING tag, uuid_sync, deleted_at, updated_at
      `;
      if (rows.length === 0) return res.status(404).json({ success: false, message: 'Equipo no encontrado' });
      return res.json({ success: true, data: rows[0], softDeleted: true });
    }
