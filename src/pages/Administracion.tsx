...
export default function Administracion() {
  ...
  const activeUsers = users.filter(u => u.nombre?.toLowerCase().includes(filter.toLowerCase()) || u.email?.toLowerCase().includes(filter.toLowerCase()));
    ...
    <div className="flex flex-col gap-8 text-left animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button 
            onClick={() => activeTab === 'users' ? handleAddUser() : handleAddClient()}
