...
const getMobileSection = (href: string) => {
  if (["/", "/scanner", "/equipos", "/mapa"].includes(href)) return "Operación";
  if (["/clientes", "/inventario", "/mantenimientos", "/planificacion", "/ordenes-servicio", "/tickets"].includes(href)) return "Gestión";
  if (["/reportes", "/eficiencia"].includes(href)) return "Análisis";
  ...
  if (variant === 'large') {
    return (
      <Link href={href} onClick={onClick}>
        <div className={`mx-4 my-1.5 flex min-h-14 items-center gap-4 rounded-2xl border-2 p-4 cursor-pointer transition-all active:scale-95 ${
          isActive 
          ...
          <Icon className={`h-6 w-6 ${isActive ? "text-white" : isDarkMode ? "text-slate-100" : "text-slate-900"}`} />
          <span className="flex-1 whitespace-nowrap text-xs font-black uppercase tracking-[0.08em]">{label}</span>
  ...
  return (
    <div className={`h-screen w-full flex overflow-hidden font-sans ${isDarkMode ? 'dark bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
      ...
      <>
        ...
        {isMobileMenuOpen && (
          ...
          >
            ...
            <nav className="flex-1 overflow-y-auto py-4 custom-scrollbar" aria-label="Navegación principal">
              {visibleNavItems.map((item, index) => {
                const section = getMobileSection(item.href);
                const previousSection = index > 0 ? getMobileSection(visibleNavItems[index - 1].href) : null;
                return (
                  <div key={item.href}>
                    {section !== previousSection && (
                      <h2 className={`mx-5 mb-1 mt-4 text-[10px] font-black uppercase tracking-[0.2em] ${
              ...
              <button
                type="button"
                onClick={openSyncInspector}
                className={`mx-4 my-1.5 flex min-h-14 w-[calc(100%-2rem)] items-center gap-4 rounded-2xl border-2 p-4 text-left transition-all active:scale-95 ${
                  isDarkMode
              ...
              >
                ...
                <span className="flex-1 text-xs font-black uppercase tracking-[0.08em]">Sync Inspector</span>
                {stats.offlineOps > 0 && (
              ...
              >
                ...
                <span className="flex-1 text-xs font-black uppercase tracking-[0.08em]">Sincronizar ahora</span>
                {stats.offlineOps > 0 && (
                  <span className="flex h-8 min-w-8 items-center justify-center rounded-full bg-amber-500 px-2 text-[10px] font-black text-white">
                    {stats.offlineOps}
                  </span>
                )}
              </button>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        ...
        <header className={`h-16 shrink-0 border-b flex items-center justify-between px-4 lg:px-8 z-30 shadow-sm ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <button 
              ...
              onClick={toggleTheme} 
              className={`flex h-11 w-11 items-center justify-center rounded-xl border transition-colors focus-visible:ring-2 focus-visible:ring-blue-500 ${isDarkMode ? 'border-slate-800 hover:bg-slate-800 text-slate-400' : 'border-slate-200 hover:bg-slate-100 text-slate-600'}`}
            >
            ...
            <button
              ...
              className={`flex min-h-11 items-center gap-1.5 rounded-full border px-3 py-2 text-[10px] font-black tracking-wider uppercase focus-visible:ring-2 focus-visible:ring-blue-500 ${
              isDarkMode 
            ...
            <div className={`flex items-center gap-3 border-l pl-4 ${isDarkMode ? 'border-slate-800' : 'border-slate-200'}`}>
              ...
              <div className="relative">
                <div className="flex h-11 w-11 items-center justify-center rounded-full bg-blue-600 text-xs font-bold text-white shadow-lg shadow-blue-600/30 ring-2 ring-blue-600/20">
                  NB
                </div>
            ...
            <button 
              ...
              className={`flex h-11 min-w-11 shrink-0 items-center justify-center gap-1.5 rounded-xl border p-2.5 text-[10px] font-black uppercase shadow-sm transition-all duration-300 group cursor-pointer active:scale-95 focus-visible:ring-2 focus-visible:ring-red-500 ${
                isDarkMode 
        ...
        {/* Main Content Area */}
        <main className={`flex-1 min-w-0 app-shell-main overflow-y-auto overflow-x-hidden p-4 pb-44 lg:p-8 relative ${isDarkMode ? 'bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
          {children}
          <footer
        ...
        <nav className={`lg:hidden fixed bottom-6 ${menuPosition === 'right' ? 'right-6' : 'left-6'} z-50 flex items-center gap-3`}>
          ...
          >
            ...
            <Link href="/mantenimientos" aria-label="Abrir mantenimientos" title="Mantenimientos">
            ...
            <Link href="/scanner" aria-label="Abrir escáner QR" title="Escáner QR">
            ...
            <Link href="/" aria-label="Abrir dashboard" title="Dashboard">
