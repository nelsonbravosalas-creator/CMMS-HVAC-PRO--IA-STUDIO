 ...
 *
 * Fuente de verdad funcional para escalar módulos, botones, formularios y flujos offline-first.
 */
export const BUSINESS_RULES_VERSION = '2026.05-offline-first-v4';

export const NORMALIZED_STATES = {
  assets: ['operativo', 'falla', 'mantenimiento', 'baja'] as const,
  work_orders: ['abierto', 'asignado', 'en_proceso', 'resuelto', 'cerrado', 'cancelado'] as const,
  preventive_maintenance: ['programado', 'en_proceso', 'ejecutado', 'vencido', 'cancelado', 'observado'] as const,
  reports: ['borrador', 'firmado', 'bloqueado', 'anulado'] as const,
  ordenes_servicio: ['borrador', 'firmada', 'enviada', 'anulada'] as const
} as const;
...
export const BUTTON_WORKFLOW_RULES = {
  create: {
    ...
    workflow: ['deshabilitar botón', 'validar formulario', 'crear uuid_sync', 'guardar local pending_insert', 'encolar insert', 'disparar sync', 'mostrar resultado'] as const,
    uiState: 'El botón puede operar offline, debe bloquear doble click e informar guardado local.'
  },
  edit: {
    intent: 'Modificar registro existente.',
    workflow: ['deshabilitar botón', 'validar cambios', 'actualizar updated_at', 'guardar local pending_update', 'encolar update', 'disparar sync', 'mostrar resultado'] as const,
  ...
  delete: {
    ...
    workflow: ['confirmar impacto', 'deshabilitar botón', 'set deleted_at', 'guardar local pending_delete', 'encolar delete', 'disparar sync', 'mostrar estado pendiente'] as const,
    uiState: 'El botón debe pedir confirmación y mostrar estado pendiente hasta confirmación server.'
  },
  finalize: {
    intent: 'Cerrar documento operativo como informe u orden de servicio.',
    workflow: ['deshabilitar botón', 'validar datos mínimos', 'generar folio/id', 'cambiar estado firmado', 'guardar local', 'encolar sync', 'eliminar borrador local', 'mostrar confirmación'] as const,
    uiState: 'Debe bloquear doble click y conservar borrador si ocurre error recuperable.'
  },
  sync: {
...
export const ROLE_RULES = {
  Administrador: { canConfigureSystem: true, canManageUsers: true, canDeleteSyncedRecords: true, canRunDbHealth: true, canForceImport: true },
  Técnico_Líder: { canConfigureSystem: false, canManageUsers: false, canDeleteSyncedRecords: true, canRunDbHealth: false, canForceImport: false },
  Ingeniero_Confiabilidad: { canConfigureSystem: false, canManageUsers: false, canDeleteSyncedRecords: false, canRunDbHealth: false, canForceImport: false },
  Técnico: { canConfigureSystem: false, canManageUsers: false, canDeleteSyncedRecords: false, canRunDbHealth: false, canForceImport: false }
} as const;

export const MODULE_RULES = {
  assets: {
    label: 'Activos HVAC', table: 'assets', primaryIdentifier: 'uuid_sync', businessIdentifier: 'tag',
    requiredFields: ['tag', 'nombre', 'estado'] as const,
    searchableFields: ['tag', 'nombre', 'marca', 'modelo', 'serie', 'ubicacion'] as const,
    allowedStates: NORMALIZED_STATES.assets,
    buttons: { create: 'Crear activo', edit: 'Editar activo', delete: 'Dar de baja', sync: 'Sincronizar activos', view: 'Ver activo' }
  },
  work_orders: {
    label: 'Órdenes de trabajo', table: 'work_orders', primaryIdentifier: 'uuid_sync', businessIdentifier: 'id',
    requiredFields: ['titulo', 'prioridad', 'estado', 'equipo_tag'] as const,
    allowedStates: NORMALIZED_STATES.work_orders,
    buttons: { create: 'Crear OT', edit: 'Actualizar OT', delete: 'Anular OT', sync: 'Sincronizar OT', view: 'Ver OT' }
  },
  preventive_maintenance: {
    label: 'Mantenimiento preventivo', table: 'preventive_maintenance', primaryIdentifier: 'uuid_sync', businessIdentifier: 'id',
    requiredFields: ['equipo_tag', 'tecnico', 'tipo', 'fecha', 'estado'] as const,
    allowedStates: NORMALIZED_STATES.preventive_maintenance,
    buttons: { create: 'Programar mantenimiento', edit: 'Actualizar mantenimiento', delete: 'Cancelar mantenimiento', sync: 'Sincronizar mantenimiento', view: 'Ver mantenimiento' }
  },
  clients: { label: 'Clientes', table: 'clients', primaryIdentifier: 'uuid_sync', businessIdentifier: 'id', requiredFields: ['nombre'] as const, buttons: { create: 'Crear cliente', edit: 'Editar cliente', delete: 'Desactivar cliente', sync: 'Sincronizar clientes', view: 'Ver cliente' } },
  branches: { label: 'Sucursales', table: 'branches', primaryIdentifier: 'uuid_sync', businessIdentifier: 'id', requiredFields: ['nombre', 'cliente_id'] as const, buttons: { create: 'Crear sucursal', edit: 'Editar sucursal', delete: 'Desactivar sucursal', sync: 'Sincronizar sucursales', view: 'Ver sucursal' } },
  users: { label: 'Usuarios', table: 'users', primaryIdentifier: 'uuid_sync', businessIdentifier: 'id', requiredFields: ['nombre', 'rol', 'pin', 'activo'] as const, buttons: { create: 'Crear usuario', edit: 'Editar usuario', delete: 'Desactivar usuario', sync: 'Sincronizar usuarios', view: 'Ver usuario' } },
  reports: { label: 'Informes', table: 'reports', primaryIdentifier: 'uuid_sync', businessIdentifier: 'id', requiredFields: ['id', 'data'] as const, allowedStates: NORMALIZED_STATES.reports, buttons: { create: 'Crear informe', edit: 'Editar informe', delete: 'Anular informe', sync: 'Sincronizar informes', view: 'Ver informe' } },
  events: { label: 'Eventos', table: 'events', primaryIdentifier: 'uuid_sync', businessIdentifier: 'id', requiredFields: ['id', 'data'] as const, buttons: { create: 'Crear evento', edit: 'Editar evento', delete: 'Anular evento', sync: 'Sincronizar eventos', view: 'Ver evento' } },
  ordenes_servicio: { label: 'Órdenes de servicio', table: 'ordenes_servicio', primaryIdentifier: 'uuid_sync', businessIdentifier: 'id', requiredFields: ['id', 'data'] as const, allowedStates: NORMALIZED_STATES.ordenes_servicio, buttons: { create: 'Guardar y finalizar O.S.', edit: 'Editar O.S.', delete: 'Anular O.S.', sync: 'Sincronizar O.S.', view: 'Ver O.S.' } }
} as const;
